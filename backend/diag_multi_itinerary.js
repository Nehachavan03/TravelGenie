...
const testId = (index) => {
  ...
  db.query(headerQuery, [id], (err, headerResults) => {
    if (err) {
      console.error('Header Error for ID', id, ':', err.message);
    } else {
      console.log('Header Results for ID', id, ':', headerResults.length > 0 ? 'FOUND' : 'NOT FOUND');
    }
    ...
    db.query(itemsQuery, [id], (err, itemsResults) => {
      if (err) {
        console.error('Items Error for ID', id, ':', err.message);
      } else {
        console.log('Items Results for ID', id, ':', itemsResults.length);
      }
      testId(index + 1);
    });
  });
